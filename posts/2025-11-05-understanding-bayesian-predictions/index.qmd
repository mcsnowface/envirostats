...
# This gives E[μᵢ|D], which equals E[ỹᵢ|D] for Normal models
...
kable(y_pred_matrix,
      digits = 2,
      caption = "Posterior predictive samples from p(ỹ|D)")
```
...
# - add_epred_draws(): posterior distribution of mean response p(μ|D)
# - add_predicted_draws(): posterior predictive distribution p(ỹ|D)
...
# In logistic regression: probability = logit⁻¹(linear predictor)
...
results <- c(
  mean_then_transform = plogis(mean(log_odds)),   # logit⁻¹(E[η]) = 0.50
  transform_then_mean = mean(plogis(log_odds))    # E[logit⁻¹(η)] ≈ 0.60
)
