...
![From uncertainty, form arises — the flow from belief to prediction.](bayesian_prediction.png)
...
> **Note:** All examples in this post were built using R with the `rstanarm`, `tidybayes`, and `ggplot2` packages. You can follow along by copying and pasting the code -- everything is self-contained and reproducible.
...
Once you have posterior samples from $p(\theta\mid D)$, you can simulate the posterior predictive distribution $p(\tilde{y}\mid D)$ and summarize it with its mean $E[\tilde{y}\mid D]$. We’ll return to this trio when it becomes most helpful.
...
Suppose you’re a fish biologist interested in the hatching rate of rainbow trout. You run an experiment and **4 out of 10 eggs hatched** -- what’s the hatching rate $\theta$? With one parameter, a **grid approximation** shows the mechanics explicitly.
...
# 👉 CHANGE THIS LINE to try different priors:
prior <- prior_flat   

# Likelihood: how likely is observing 4/10 for each possible θ?
...
# Real analyses use 1,000–4,000+ draws.

predicted_matrix <- matrix(c(
  2.0, 2.1, 1.9,  # Obs 1: μ₁ from posterior draws 1, 2, 3
  1.0, 1.2, 1.1,  # Obs 2: μ₂ from posterior draws 1, 2, 3
  4.0, 3.8, 4.2,  # Obs 3: μ₃ from posterior draws 1, 2, 3
  1.0, 1.1, 0.9   # Obs 4: μ₄ from posterior draws 1, 2, 3
), nrow = 4, byrow = TRUE)
...
- Next we’ll add $\sigma$ to generate draws of $\tilde{y}$ ⇒ $p(\tilde{y}\mid D)$ and prediction intervals
...
Plain language: posterior over parameters ⇒ simulate data from it ⇒ average those simulations.
...
# Expected predictions: mean of μᵢ across posterior draws
...
# Posterior draws of σ (one for each column)
sigma_draws <- c(0.5, 0.6, 0.55)

# Generate ỹᵢ ~ Normal(μᵢ, σ) for each draw
...
kable(y_pred_matrix,
      digits = 2,
      caption = "Posterior predictions: samples from p(ỹ|D)")
```
...
- `(Intercept)`: mean = 1.9 (we simulated with 2.0) ✓
- `x`: mean = 0.4 (we simulated with 0.5) ✓
- `sigma`: mean = 1.0 (we simulated with sd = 1) ✓
...
ppc_dens_overlay(y, y_rep[1:100, ]) +
  labs(title = "Posterior predictive check: Density comparison",
       subtitle = "Light blue = 100 draws from p(ỹ|D), Dark blue = observed data") +
  theme_minimal()
```
...
# - add_epred_draws(): expected predictions E[ỹ|D]
# - add_predicted_draws(): full posterior predictive samples p(ỹ|D)
# Let's visualize both simultaneously:

data |>
  add_epred_draws(fit) |>
  ggplot(aes(x = x, y = y)) +
  # Blue bands: credible intervals for E[μ|D]
  stat_lineribbon(aes(y = .epred), .width = c(0.61, 0.89)) +
  # Black points: observed data
  geom_point(data = data, size = 2, alpha = 0.8) +
  # Red points: 50 samples from p(ỹ|D) showing full predictive distribution
  ...
  labs(title = "Expected predictions (blue) vs. Posterior predictions (red)",
       subtitle = "Blue bands: E[μ|D] credible intervals; Red: samples from p(ỹ|D)",
...
p1 <- ggplot(epred_intervals, aes(x = x, y = .epred)) +
  ...
  labs(title = "89% Credible Intervals for E[μ|D]",
...
p2 <- ggplot(pred_intervals, aes(x = x, y = .prediction)) +
  ...
  geom_point(data = data, aes(x = x, y = y), alpha = 0.3, size = 1) +
  labs(title = "89% Prediction Intervals for p(ỹ|D)",
...
- Posterior $p(\theta\mid D)$ ⇒ simulate the linear predictor $\mu$ for each draw  
