...
We do not observe $\psi_i$ directly either, but we model it as a function of site-level covariates, such as things like noise level and distance to the nearest park:
...
The two approaches are not mutually exclusive. You can embed an occupancy model inside a multilevel structure (for example, if sites are nested within neighbourhoods and you want neighbourhood-level random effects on $\psi$), in which case both kinds of hierarchy operate simultaneously.
...
- *Variation in detection covariates.* If you have covariates that affect $p$ but not $\psi$ (for example, time of day affects whether someone answers but does not change whether they live there), this gives the model a dedicated lever to separate the two processes. It is the single most powerful design choice you can make.
...
On the logit scale, with a standardised continuous noise covariate (so that a one-unit increase represents a meaningful shift in noise level), this corresponds to $\alpha_0 \approx 0.40$ (the baseline occupancy intercept) and $\alpha_1 \approx -1.20$ (the noise slope). These parameters are explained in detail in the Stan model section below.
...
The logit scale is a transformed scale where 0 corresponds to a probability of 0.50, positive values correspond to probabilities above 0.50, and negative values correspond to probabilities below 0.50. To convert back to a probability, you apply the logistic function: $$\text{logit}^{-1}(x) = e^x / (1 + e^x)$$

$\alpha_0$ and $\beta_0$ are intercepts on this scale and need to be converted to be interpreted as probabilities. $\alpha_1$ is a slope and works differently: it describes a constant change in log-odds per one-unit increase in standardised noise. Because the logistic function produces a sigmoidal curve, this does not translate to a fixed change in probability. The change in probability per unit of noise depends on where you are on the curve, being largest near 50% occupancy and smallest near the extremes.

So what do the true values, the parameters we used to simulate the data, mean in plain terms?

- $\alpha_0 = 0.40$: the true baseline occupancy intercept. Converting: $\text{logit}^{-1}(0.40) \approx 0.60$, meaning a site with average noise has roughly 60% occupancy probability.
- $\alpha_1 = -1.20$: the true noise slope. **Negative** because higher noise is associated with lower occupancy. A one-unit increase in standardised noise reduces the log-odds of occupancy by 1.20. Because of the sigmoidal curve this corresponds to different probability drops depending on the baseline, being largest near 50% occupancy and smallest near the extremes.
- $\beta_0 = 0.40$: the true detection intercept. Converting: $\text{logit}^{-1}(0.40) \approx 0.60$, corresponding to a true detection probability of 60%.

If noise had no effect on occupancy, $\alpha_1$ would be near zero. If noise made neighbourhoods *more* occupied, it would be positive.

The figure below shows the posterior distribution for each parameter, representing the model's full range of uncertainty after seeing the data, with the true simulation values marked as dashed red lines. A narrow distribution means the data pin down the parameter precisely; a wide one means more uncertainty remains.


```{r}
#| label: fig-parameter-recovery
#| fig-cap: "Posterior distributions for the three model parameters, all on the  logit scale. Dashed red lines mark the true values used to simulate the data, not the posterior means. $\\alpha_0$ and $\\alpha_1$ are recovered closely. $\\beta_0$ shows a slight offset, the posterior is centred around 0.23 rather than the true 0.40, which is normal sampling variation: detection probability is harder to pin down precisely because it can only be estimated from visits to genuinely occupied units."
#| code-fold: true
...
$\alpha_0$ and $\alpha_1$ are recovered closely, as expected given the strong signal from the contrast between quiet and noisy neighbourhoods. $\beta_0$ shows a slightly larger offset from the true value: the posterior mean is approximately 0.23 compared to the true 0.40, corresponding to an estimated detection probability of about 56% versus the true 60%. This is normal sampling variation rather than a model failure. Detection probability is estimated from a smaller effective sample since it can only be informed by visits to genuinely occupied units, giving it less signal than the occupancy parameters.

### Occupancy Probability vs. Noise

The quantity we care most about is how occupancy probability varies with noise level. The figure below shows the posterior mean occupancy curve and its 90% *credible interval* (the range within which the true value falls with 90% posterior probability) across the full range of observed noise values. The logit-scale estimates have been converted back to the probability scale via the logistic function, so the y-axis shows occupancy as a probability between 0 and 1 and is directly interpretable. Rug marks along the top edge show individual quiet sites (blue); rug marks along the bottom edge show individual noisy sites (red).
...
In **Part 2**, we ask a different question: not *whether* a unit is occupied, but *how many people* are typically home. This turns the latent binary state into a latent count, and calls for an **N-mixture model**. The detection problem reappears in the same form, but the stakes are higher: with counts, not accounting for detection probability systematically deflates abundance estimates, and the bias compounds as counts get larger.
