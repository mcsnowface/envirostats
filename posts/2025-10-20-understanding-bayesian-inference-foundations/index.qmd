...
This is precisely the kind of question Bayesian inference helps us answer. In a [previous post](https://www.envirostats.ca/posts/2025-10-12-understanding-frequentist-inference/), we explored frequentist inference -- a framework built on the concept of repeated sampling, where parameters are treated as fixed unknowns and probability statements describe what would happen across hypothetical repetitions of an experiment.
...
All statistical methods involve choices (which test to use, what α-level, when to stop collecting data). Bayesian methods make these assumptions explicit and transparent. Moreover, with sufficient data, different reasonable priors converge to similar posteriors [@berger2006case].
...
$$
\text{Beta}(\alpha, \beta) + \text{Data}(k \text{ germinate}, n-k \text{ fail}) = \text{Beta}(\alpha + k, \beta + n - k)
$$
...
# Frequentist scenario: testing H0: θ = 0.70 (acceptable germination)
# We'll check after every 20 seeds

check_points <- seq(20, 100, by = 20)
frequentist_checks <- tibble(
  seeds_tested = check_points,
  germinated = numeric(length(check_points)),
  p_value = numeric(length(check_points)),
  decision_unadjusted = character(length(check_points)),
  decision_bonferroni = character(length(check_points))
)

for (i in seq_along(check_points)) {
  n <- check_points[i]
  germ_observed <- sum(germinations[1:n])
  
  # Test H0: θ = 0.70 (acceptable quality) vs H1: θ < 0.70
  p_val <- binom.test(germ_observed, n, p = 0.70, alternative = "less")$p.value
  
  frequentist_checks$germinated[i] <- germ_observed
  frequentist_checks$p_value[i] <- p_val
  
  # Decision with α = 0.05 (WRONG - doesn't account for multiple looks)
  frequentist_checks$decision_unadjusted[i] <- ifelse(p_val < 0.05, 
                                                       "Reject batch", 
                                                       "Continue testing")
  
  # Decision with Bonferroni correction: α = 0.05/5 = 0.01
  frequentist_checks$decision_bonferroni[i] <- ifelse(p_val < 0.01, 
                                                       "Reject batch", 
                                                       "Continue testing")
}

kable(frequentist_checks,
      digits = 4,
      col.names = c("Seeds", "Germinated", "p-value",
                    "Decision (α=0.05)", "Decision (Bonferroni)"),
      caption = "Frequentist sequential testing showing the impact of multiple testing correction")
```
<br>
...
**In Bayesian inference**: Your posterior after testing 100 seeds is identical whether you:
...
We've covered the foundations of Bayesian inference through a simple but powerful example: estimating a probability from coin flips. We've seen how priors encode beliefs, how data updates those beliefs into posteriors, how credible intervals directly quantify uncertainty, and how posterior predictive distributions let us make probabilistic forecasts.

Throughout this post, we've occasionally compared Bayesian and frequentist approaches to help build intuition. **From here forward, we'll focus exclusively on Bayesian methods**, exploring their full power and flexibility on their own terms.
...
The shift from thinking about procedures to thinking about beliefs is the essence of Bayesian inference. Once you internalize this shift through simple examples like coin flips, you're ready to apply the same reasoning to arbitrarily complex models.
